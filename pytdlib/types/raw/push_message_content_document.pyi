from .document import Document
from .push_message_content import PushMessageContent

### FILE IS AUTOGENERATED. DO NO EDIT IT BY HAND ###

class PushMessageContentDocument(PushMessageContent):
    """
    A document message (a general file)

    :param document: Message content; may be null
    :param is_pinned: True, if the message is a pinned message with the specified content
    """
    __slots__ = ("document", "is_pinned", "_extra", "_client_id", "_type")

    def __init__(self, document: Document | None = None, is_pinned: bool = False):
        self.document = document
        self.is_pinned = is_pinned
        self._type = "pushMessageContentDocument"