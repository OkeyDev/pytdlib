from .object_base import ObjectBase
from .reaction_notification_source import ReactionNotificationSource

### FILE IS AUTOGENERATED. DO NO EDIT IT BY HAND ###

class ReactionNotificationSettings(ObjectBase):
    """
    Contains information about notification settings for reactions

    :param message_reaction_source: Source of message reactions for which notifications are shown
    :param story_reaction_source: Source of story reactions for which notifications are shown
    :param sound_id: Identifier of the notification sound to be played; 0 if sound is disabled
    :param show_preview: True, if reaction sender and emoji must be displayed in notifications
    """
    __slots__ = ("message_reaction_source", "story_reaction_source", "sound_id", "show_preview", "_extra", "_client_id", "_type")

    def __init__(self, message_reaction_source: ReactionNotificationSource | None = None, story_reaction_source: ReactionNotificationSource | None = None, sound_id: int = 0, show_preview: bool = False):
        self.message_reaction_source = message_reaction_source
        self.story_reaction_source = story_reaction_source
        self.sound_id = sound_id
        self.show_preview = show_preview
        self._type = "reactionNotificationSettings"