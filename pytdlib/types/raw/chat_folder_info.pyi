from .chat_folder_icon import ChatFolderIcon
from .chat_folder_name import ChatFolderName
from .object_base import ObjectBase

### FILE IS AUTOGENERATED. DO NO EDIT IT BY HAND ###

class ChatFolderInfo(ObjectBase):
    """
    Contains basic information about a chat folder

    :param id: Unique chat folder identifier
    :param name: The name of the folder
    :param icon: The chosen or default icon for the chat folder
    :param color_id: The identifier of the chosen color for the chat folder icon; from -1 to 6. If -1, then color is disabled
    :param is_shareable: True, if at least one link has been created for the folder
    :param has_my_invite_links: True, if the chat folder has invite links created by the current user
    """
    __slots__ = ("id", "name", "icon", "color_id", "is_shareable", "has_my_invite_links", "_extra", "_client_id", "_type")

    def __init__(self, id: int = 0, name: ChatFolderName | None = None, icon: ChatFolderIcon | None = None, color_id: int = 0, is_shareable: bool = False, has_my_invite_links: bool = False):
        self.id = id
        self.name = name
        self.icon = icon
        self.color_id = color_id
        self.is_shareable = is_shareable
        self.has_my_invite_links = has_my_invite_links
        self._type = "chatFolderInfo"